...
Representa o *valor central do sinal* ou a sua componente contínua (DC).

$ mu = 1/N sum_(n=0)^(N-1) x[n] $

Significado: indica o nível de deslocamento (offset) do sinal em relação ao zero.

== Variância

Mede a *dispersão do sinal em relação à sua média*. 

$ sigma^2 = 1/N sum_(n=0)^(N-1)(x[n]-mu)^2 $

Significado: Quantifica o quanto o sinal oscila ao redor da média. Quanto maior a variância, mais ruidoso ou dinâmico é o sinal.

== Valor Eficaz / RMS (Root Mean Square)

Raiz quadrada da média dos quadrados do sinal. Corresponde ao valor de tensão ou corrente contínua equivalente que produziria a mesma dissipação de potência em uma carga resistiva.

$ x_"RMS" = 1/N sum_(n=0)^(N-1) x[n]^2 $

Relação com a média e variância: $ x_"RMS" = mu^2 + sigma^2 $ (Potência total = Potência DC + Potência AC)

== Autocorrelação ($R_(x x)[k]$)

Mede o *grau de similaridade entre o sinal $x[n]$ e uma versão deslocada no tempo de si mesmo por $k$ amostras*.

$ R_(x x)[k] = sum_n x[n] dot x[n+k] $

Significado: Revela frequências fundamentais e ecos escondidos no sinal, mesmo na presença de ruído.

= Domínio da Frequência

== FFT (Fast Fourier Transform)

Transforma todo o sinal do tempo diretamente para o domínio da frequência, decompondo em uma soma de senos e cossenos.

$ X[k] = sum_(n=0)^(N-1) x[n] e^(-j (2 pi)/N k n) $

A limitação é que se perde a informação temporal.

== STFT (Short-Time Fourier Transform)

Para contornar a limitação da FFT em sinais não estacionários (como voz ou música), a STFT aplica a FFT em pequenas janelas de tempo deslizando sobre o sinal.

$ X(t,f) = integral x(tau)w(tau-t)e^(-j 2pi f tau) d tau $

Basicamente, divide-se o sinal em blocos e calcula a FFT para cada bloco. Como resultado, gera-se um *espectograma* (tempo vs frequência vs amplitude).

Janelas curtas dão boa resolução temporal, ams baixa resolução em frequência. Janelas longas dão o oposto.

== Wavelets (Transformada Wavelet)

Em vez de usar senos e cossenos infinitos como a Fourier, a transformada Wavelet decompõe o sinal usando "pequenas ondas" (wavelets) com duração e frequência limitadas.

$ W T(a,b) = 1/(sqrt(|a|)) integral x(t) phi^* ((t-b)/a) d t $

Utiliza janelas de tamanho variável:

- Frequências altas: Janelas temporais estreitas (alta resolução no tempo)
- Frequências baixas: Janelas temporais largas (alta resolução em frequência)

Resolve o dilema da STFT, adaptando a resolução conforme a faixa de frequência.

== PSD (Power Spectral Density)

Indica como a potência do sinal está distribuída ao longo das frequências.

$ P_(x x)(f)= lim_(t arrow.r infinity) 1/T |X_T (f)|^2 $

É a transformada de Fourier da função de autocorrelação do sinal (Teorema de Wiener-Khinchin).

= Filtragem

== Low-Pass

Deixa passar frequẽncias abaixo de uma frequência de corte e atenua as frequências acima dela.

Aplicação: Remover ruído elétrico agudo em gravações de voz ou suavizar leituras de sensores (como acelerômetros).

== High-Pass

Deixa passar frequências acima de uma frequência de corte e atenua as frequências abaixo dela.

Aplicação: Eliminar frequência DC da alimentação.

== Notch

Atenua fortemente uma faixa muito estreita de frequências ao redor de uma frequência central, deixando passar todo o resto.

Aplicação: clássico filtro Notch de 60Hz para remover a hum elétrico da tomada captado por sensores.

== Passa-Faixa (Band-Pass)

Deixa passar apenas as frequências contidas entre duas frequências de corte.

Aplicação: Sintonia de rádio ou isolar ritmos específicos de um sinal de EEG (ex: ondas Alfa entre 8Hz e 12Hz)

= Exemplo em Python

Vamos usar as bibliotecas: `pip install numpy scipy pywavelets matplotlib`.
...
potencia_psd_integrada = np.trapz(psd, f_psd) # Área sob a curva da PSD
...
== Output do Código (Texto + Gráfico)

```plain
==================================================
 1. MÉTRICAS ÚNICAS NO DOMÍNIO DO TEMPO
==================================================
• Média (Componente DC)           : 1.9986
• Variância (Potência AC)         : 5.8764
• Desvio Padrão                   : 2.4241
• RMS (Valor Eficaz)              : 3.1418
• Potência Média (RMS²)           : 9.8708
• Energia Total do Sinal          : 9870.7548
• Autocorrelação em Lag Zero Rxx[0]: 9870.7548

==================================================
 2. MÉTRICAS ÚNICAS NO DOMÍNIO DA FREQUÊNCIA
==================================================
• Frequência Dominante (FFT)       : 10.00 Hz
• Maior Amplitude Espectral (FFT)  : 2.9968
• Potência Integrada da PSD        : 5.8030 V²
• Energia Total nos Coefs Wavelet : 15479790.4789

==================================================
 3. IMPACTO DOS FILTROS (MÉDIAS E RMS PÓS-FILTRO)
==================================================
• Passa-Baixa  (<20Hz)   -> Média: 1.9948 | RMS: 2.9045
• Passa-Alta   (>5Hz)    -> Média: 0.0359 | RMS: 2.4003 (DC Removido)
• Notch        (60Hz)    -> Média: 2.0004 | RMS: 2.9615
• Passa-Faixa  (8-12Hz)  -> Média: 0.0293 | RMS: 1.8637
```

#figure(image("graph.png"))